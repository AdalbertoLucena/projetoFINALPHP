<?php/*
session_start();

if (!isset($_SESSION['usuario_id'])) {
    header("Location: ../cadastro_formulario.php");
    exit;
}*/
?>


<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Hyundai - Alugar</title>
    <link rel="stylesheet" href="hyundai.css">
</head>
<body>
    <main class="car-page">
        <div class="card">
            <h1 class="car-title">MODELO DO CARRO</h1>
            <div class="image-wrap">
                <img class="car-image" src="https://www.hyundai.com.br/content/dam/hmb/product-page/novo-hyundai-hb20s/design/features/modal/desktop/03-design_rodas_desk.webp" alt="CARRO">
            </div>
            <p class="price">R$ 200,00</p>

            <div class="description">
                <p>Descrição curta:  Hyundai HB20S é um sedã compacto que se destaca pelo seu design moderno, bom pacote de equipamentos e uma motorização turbo eficiente nas versões mais caras, sendo uma opção popular no mercado brasileiro.</p>
                <ul class="features">
                    <li><strong>Ano:</strong> 2024</li>
                    <li><strong>Combustível:</strong> Flex</li>
                    <li><strong>Câmbio:</strong> Automático</li>
                    <li><strong>Passageiros:</strong> 5</li>
                </ul>
            </div>

            <form class="reserve-form" method="post" action="reserva.php">
                <input type="hidden" name="modelo" value="Hyundai HB20S Turbo">
                <input type="hidden" name="diaria" value="200">
                <label for="dias">Diárias</label>
                <input id="dias" name="dias" type="number" min="1" value="1" class="dias-input">
                <button type="submit" class="btn btn-primary">FINALIZAR</button>
            </form>
        </div>
    </main>
</body>
</html>